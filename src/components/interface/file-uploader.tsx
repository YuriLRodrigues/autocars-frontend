...
interface FileUploaderProps extends React.HTMLAttributes<HTMLDivElement> {
   ...
   */
  onUpload?: (file: File) => Promise<ActionResponse<{ id: string; url: string }[]>>
  ...
  setResponseValues: (values: { id: string; url: string }[]) => void
}
      ...
      if (onUpload) {
        try {
          ...
          switch (_error.message) {
            case 'Unauthorized':
              toast.error('Você deve estar logado para enviar uma imagem.')
              break
            default:
              toast.error(`Erro ao enviar a imagem: ${_error.message}`)
          }
          setFiles([])
        }
      }
    },
    [onUpload, setFiles, setResponseValues, maxSize],
  )
          ...
          >
            ...
            {isDragActive ? (
              <div className="flex flex-col items-center justify-center gap-4 sm:px-5">
                ...
                <p className="font-medium text-muted-foreground">Arraste o(s) arquivo(s) aqui.</p>
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center gap-4 sm:px-5">
                <div className="rounded-full border border-dashed p-3">
