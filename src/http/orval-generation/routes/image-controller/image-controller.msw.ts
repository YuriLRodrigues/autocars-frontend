...
export const getUploadAvatarResponseMock = (overrideResponse: Partial<AvatarDto> = {}): AvatarDto => ({
...
export const getUploadAvatarMockHandler = (
  overrideResponse?:
    | AvatarDto
    | ((info: Parameters<Parameters<typeof http.post>[1]>[0]) => Promise<AvatarDto> | AvatarDto),
) => {
  return http.post('*/image/upload/avatar', async (info) => {
    await delay(1000)
